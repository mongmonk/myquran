      ...
      <ul class="list-reset bold m2 p2 ampstart-label column-2">
        <?php foreach($data as $i => $surat){?>
          <li class="ampstart-nav-item ampstart-nav-dropdown relative mb2">
            ...
            >
              <section>
                <header>(<?php echo $surat->name ?>) <?php echo $surat->englishName ?> <?php echo count($surat->ayahs) ?> ayat</header>
                <ul class="ampstart-dropdown-items m0 p1 column-2">
                  <?php 
                      foreach($surat->ayahs as $i => $ayat){
                          echo "<li class=\"ampstart-dropdown-item mb1\"><a href='/ayah/{$ayat->number}'>Ayat {$ayat->numberInSurah}</a></li>";
                      }
                  ?>
                </ul>
              </section>
    ...
    <footer class="ampstart-footer flex flex-column items-center px3">
      <nav class="ampstart-footer-nav">
        <ul class="list-reset flex flex-wrap mb3">
          <li class="px1">
            <a class="text-decoration-none ampstart-label" href="/quran/about">TENTANG <?php echo SITE_NAME ?></a>
          </li>
          <li class="px1">
            <a class="text-decoration-none ampstart-label" href="/quran/contact">HUBUNGI <?php echo SITE_NAME ?></a>
          </li>
          <li class="px1">
