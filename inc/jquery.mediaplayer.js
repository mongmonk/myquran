jQuery( document ).ready( function( $ ) {
    $.alQuranMediaPlayer = {
        ...
        init: function(player, mode, firstAyah, lastAyah, surah, surahChangers, reciter) {
            this.mode = mode;
            this.player = player;
        ...
        monitorPlayer: function(number) {
            ...
            w.player.addEventListener('ended', function audioListener() {
                ...
                $('#activeAyah').attr('src', 'https://cdn.islamic.network/quran/audio/64/' + w.reciter + '/' + number + '.mp3');
                if (w.player.paused) {
                    w.player.load();
