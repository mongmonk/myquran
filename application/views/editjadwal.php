        ...
        <div id="content">
            ...
            <div class="post container">
                ...
                <form method="post" enctype="multipart/form-data">
                    <div class="form-group my-2"><label class="text-danger">Nama Masjid (max:36 karakter)*</label><input type="text" class="form-control" placeholder="Masjid Shodiqul Akbar" name="nama" maxlength="36" value="<?php echo $nama ?>" required></div>
                    <div class="form-group my-2"><label class="text-danger">Alamat Masjid (max:75 karakter)*</label><input type="text" class="form-control" placeholder="Dsn. Bakulan Ds. Bendosewu Kec. Talun Kab. Blitar" value="<?php echo $alamat ?>" name="alamat" maxlength="75" required></div>
                    <div class="form-group my-2"><label class="text-danger">Foto Masjid*</label>
                        <input type="file" class="form-control" name="img">
                        <img src='<?php echo str_replace('https://', 'https://i1.wp.com/', base_url("image/{$img}")) ?>' height='150'>
                    </div>
                        ...
                        <div class="row">
                            <div class="col-6">
                                <select class="form-control" style="height: 53px" id="mp3" name="mp3">
                                    <option value="/inc/adzan/adzan1.mp3" <?php echo $adzan == '/inc/adzan/adzan1.mp3' ? 'selected': false?>>Adzan 1</option>
                                    <option value="/inc/adzan/adzan2.mp3" <?php echo $adzan == '/inc/adzan/adzan2.mp3' ? 'selected': false?>>Adzan 2</option>
                                    <option value="/inc/adzan/adzan3.mp3" <?php echo $adzan == '/inc/adzan/adzan3.mp3' ? 'selected': false?>>Adzan 3</option>
                                    <option value="/inc/adzan/beep.mp3" <?php echo $adzan == '/inc/adzan/beep.mp3' ? 'selected': false?>>Beep</option>
                                </select>
                            </div>
                            <div class="col-6">
                                <div id="play"><audio controls class='form-control'><source id='playing' src='<?php echo $adzan ?>' title='1' type='audio/mp3'></audio></div>
                            </div>
                        </div>
                    </div>
                    <div class="form-group my-2"><label class="text-danger">Pesan 1 (max:150 karakter)*</label><input type="text" class="form-control" value="<?php echo $pesan1 ?>" placeholder="Informasi infaq bulan Oktober 2022 sebesar Rp 1.855.000,- Semoga kita senantiasa dilancarkan dalam segala urusan sehari-hari. Aamiin" name="pesan1" maxlength="150" required></div>
                    <div class="form-group my-2"><label class="text-danger">Pesan 2 (max:150 karakter)*</label><input type="text" class="form-control" value="<?php echo $pesan2 ?>" placeholder="Waktu khutbah selain Khatib DILARANG BICARA! Luruskan shaf dan rapatkan barisan!" name="pesan2" maxlength="150" required></div>
                    <div class="form-group my-2"><label class="text-danger">Cari Kota Masjid*</label><input list="kota" class="form-control" name="id" value="<?php echo $id ?>" placeholder="Ketikkan nama kota lalu pilih dari list. Bukan hanya ketik kota saja" required>
                        <datalist id="kota">
                          <?php foreach ($kota as $key => $value) {
                            echo "<option value='{$value->id}. {$value->lokasi}'/>";                                 
                          }?>
                        </datalist></div>                    
