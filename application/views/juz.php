...
<body>
    <div class="wrapper mb-3 pb-5">
        <?php $this->load->view('sidenav')?>
            <div class="post">
                <h1 class='text-center arab text-dark'>Juz <?php echo $data[0]->juz ?></h1>
                ...
                foreach ($data as $juz) {
                    ...
                    echo "<p><span class='btn btn-sm btn-secondary'>ID Translation</span> {$juz->id}</p>";
                    echo "<p><span class='btn btn-sm btn-secondary'>EN Translation</span> {$juz->en}</p>";
        ...
        <div class="container">
            ...
            <div class="row justify-content-center" id="surahConfigurator">
              <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                <select id="juzSelector" class="form-control mb-2" style="height: 54px !important;" onchange="if (this.value) window.location.href=this.value">
                    <?php 
                        foreach (range(1, 30) as $key => $value) {
                            if ($value == $this->uri->segment(2)) {
                                echo "<option value='/juz/{$value}' selected>Juz {$value}</option>";
                            } else {
                                echo "<option value='/juz/{$value}'>Juz {$value}</option>";
                            }                                
                        }
                    ?>
                </select>
              ...
              <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 text-center mb-2">
                <audio id="juzPlayer" controls autoplay class="form-control">
