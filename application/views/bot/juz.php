...
<html lang="en">
  ...
  <body>
    ...
    <div class="row justify-content-between fixed-bottom py-2 bg-light border-top">
          ...
          <select class="form-control" name="juz" onchange="this.form.submit()">
            <?php 
                foreach (range(1, 30) as $key => $value) {
                    if ($value == $this->input->get('juz')) {
                        echo "<option value='{$value}' selected>Juz {$value}</option>";
                    } else {
                        echo "<option value='{$value}'>Juz {$value}</option>";
                    }                                
                }
            ?>
          </select>
      ...
      <div class="col-4">
        <form class="sample-form" action="<?php echo $_SERVER['REQUEST_URI'] ?>" target="_top">
          <input type="hidden" name="juz" value="<?php echo $currentJuz ?>">
    ...
    <script type="text/javascript">
      ...
      $.ajax({
        url: "<?php echo base_url("bot/perjuz")?>",
        data: {id:id},
        success:function(e){}
      })
  </script>
