        ...
        <?php 
          if($data){
            foreach($data as $i => $list){?>
              <li class="mb2 px3">
                  <p class="arab text-right mb2"><?php echo $list->ar ?> ﴿<?php echo $list->surahAr ?> : <?php echo $this->quran->numConverter($list->surahNum) ?>﴾</p>
                  <p class="my1"><span class="btn">ID Translation</span> <?php echo $this->uri->segment(2) !== 'search' ? $list->id : str_ireplace($this->input->get('query'), "<b class='query'><em>{$this->input->get('query')}</em></b>", $list->id) ?></p>
                  <p class="my1 pt1"><span class="btn">EN Translation</span> <?php echo $list->en ?></p>
                  <amp-audio class="mt2" width="auto" height="50" src="https://cdn.islamic.network/quran/audio/128/ar.alafasy/<?php echo $list->numList ?>.mp3">
                    <div fallback>Your browser doesn’t support HTML5 audio</div>
                  </amp-audio>
                  <div class="dash"></div>
              </li>
        <?php }} else { echo "<p class='center btn'>Ayat Tentang `<b><em>{$this->input->get('query')}</em></b>` Tidak Ada!</p>"; }?>
      </ul>
    </main>
