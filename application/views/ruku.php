        ...
        <?php $this->load->view('sidenav')?>
            <div class="post">
                ...
                foreach ($data as $ruku) {
                    $number = $this->quran->numConverter($ruku->ayat);
                    $urutan = $ruku->numList;
                    echo "<div class='ayahAudio{$urutan}'>";
                    echo "<p class='text-right arab'>{$ruku->ar} {$number}</p>";
                    echo "<p><span class='btn btn-sm btn-secondary'>ID Translation</span> {$ruku->id}</p>";
                    echo "<p><span class='btn btn-sm btn-secondary'>EN Translation</span> {$ruku->en}</p>";
        ...
        <div class="container">
            ...
            <div class="row justify-content-center" id="surahConfigurator">
              <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12">
                <select id="surahSelector" class="form-control mb-2" style="height: 54px !important;" onchange="if (this.value) window.location.href=this.value">
                    <?php 
                        foreach ($meta as $key => $ruku) {
                            if ($this->uri->segment(2) == $ruku->ruku) {
                                echo "<option value='/ruku/{$ruku->ruku}' selected>ركوع  ﴿{$this->quran->numConverter($ruku->ruku)}﴾ , {$meta[$ruku->ruku-1]->surahAr}</option>";
                            } else {
                                echo "<option value='/ruku/{$ruku->ruku}'>ركوع  ﴿{$this->quran->numConverter($ruku->ruku)}﴾ , {$meta[$ruku->ruku-1]->surahAr}</option>";
                            }                                
                        }
                    ?>
                </select>
              ...
              <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 text-center mb-2">
                <audio id="surahPlayer" controls autoplay class="form-control">
                    <source id="activeAyah" src="https://cdn.islamic.network/quran/audio/64/ar.alafasy/1.mp3" title="1" type="audio/mp3">
                </audio>
              </div>
              <div class="col-lg-3 col-md-3 col-sm-12 col-xs-12 text-center mb-2">
